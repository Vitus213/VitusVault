...
Source: https://beatai.org/learn-ai/practices/bp01

> 说明：本版本基于浏览器提取正文后进行 Markdown 规范化重排，已尽量清理页面残留与转义字符；个别目录导航残留未做语义删改。

Claude Code 最佳实践指南

### Best Practices Guide

上下文窗口是最重要的资源，几乎所有最佳实践都围绕这个约束展开
### 前言
...
提示词约束：在 CLAUDE.md 中写明严禁修改 *_test.go / *.test.ts 文件
权限控制：在.claude/settings.json中配置"deny": ["Write(_test.go)", "Write(.test.ts)"]
Git 兜底：先提交测试代码，实现完成后用git diff检查测试文件是否被意外修改
...
Auto Memory 最佳实践
保持 MEMORY.md 精简：前 200 行是"黄金区域"，每次会话自动加载。放最重要的索引信息，详细内容拆到 topic 文件
语义化组织 topic 文件：按主题而非时间组织——debugging.md、api-conventions.md比day1.md、day2.md更有用
定期清理过时记忆：项目演进后旧记忆可能误导 Claude。用/memory定期审查，删除不再适用的内容
主动引导记忆：遇到 Claude 反复犯同一个错时，直接说"记住：这个项目的测试必须用 vitest 而不是 jest"
Worktree 隔离意识：每个 Git worktree 有独立的 memory 目录（见 9.5）。如果在 worktree 中积累了有价值的记忆，记得迁移回主 worktree
/memory命令完整功能
...
越具体越好：与其说"修复这个"，不如说"检查src/auth/middleware.ts第 45 行的 token 验证逻辑"
提供约束："不引入新的第三方依赖"、"保持与现有模式一致"
...
Worktree 创建在 /.claude/worktrees/，基于默认远程分支创建新分支。退出时：
...
添加ANTHROPIC_API_KEY到仓库 Secrets
创建工作流文件：
```bash
